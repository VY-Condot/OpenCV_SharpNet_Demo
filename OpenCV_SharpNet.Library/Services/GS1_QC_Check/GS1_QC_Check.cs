...
////        //            gradeGN = GetGrades(gnCells, roiObject.UserConfiguredGrading.GridNonuniformity.GradingData);
////        //        }
...
////        //            gradeSC = GetGrades(scPercent, roiObject.UserConfiguredGrading.SymbolContrast.GradingData);
...
////        //            gradeMOD = GetGrades(worstMod, roiObject.UserConfiguredGrading.Modulation.GradingData);
////        //        }
...
////        //            gradeFPD = GetGrades(bestFpdErrors, roiObject.UserConfiguredGrading.FixedPatternDamage.GradingData);
////        //        }

////        //        //int gradeDecode = isDecoded ? 4 : 0;
////        //        int gradeDecode = 0;
////        //        if (roiObject.UserConfiguredGrading.Decode.IsEnabled)
////        //        {
////        //            gradeDecode = isDecoded ? GetGrades(bestFpdErrors, roiObject.UserConfiguredGrading.Decode.GradingData) : 0;
////        //        }


////        //        //int gradeUEC = visualErrors <= 2 ? 4 : 2;
////        //        int gradeUEC = 2;
////        //        if (roiObject.UserConfiguredGrading.UnusedErrorCorrection.IsEnabled)
////        //        {
////        //            gradeUEC = GetGrades(visualErrors, roiObject.UserConfiguredGrading.UnusedErrorCorrection.GradingData);
////        //        }
...
////        //        //int gradeQZ = qzModules >= 0.4 ? 4 : 0;
////        //        int gradeQZ = 0;
////        //        if (roiObject.UserConfiguredGrading.QuietZone.IsEnabled)
////        //        {
////        //            gradeQZ = GetGrades(qzModules, roiObject.UserConfiguredGrading.QuietZone.GradingData);
////        //        }
    ...
    public class GS1_QC_Check : IGS1_QC_Check
    {
        ...
        public GS1_QC_CheckResult EvaluateISO15415Quality(Mat rawGrayCrop, RoiObject roiObject, Rect boundingBox, Point2f[] zxingCorners, bool isDecoded)
        {
            try
            {
                ...
                int gradeAN = 4;
                if (roiObject?.UserConfiguredGrading?.AxialNonuniformity?.IsEnabled == true)
                {
                    gradeAN = GetGrades(axialNonUniformity, roiObject.UserConfiguredGrading.AxialNonuniformity.GradingData,
                        val => val <= 0.06 ? (int)Grades.A : val <= 0.08 ? (int)Grades.B : val <= 0.10 ? (int)Grades.C : val <= 0.12 ? (int)Grades.D : (int)Grades.F);
                }
                ...
                int gradeGN = 4;
                if (roiObject?.UserConfiguredGrading?.GridNonuniformity?.IsEnabled == true)
                {
                    gradeGN = GetGrades(gnCells, roiObject.UserConfiguredGrading.GridNonuniformity.GradingData,
                        val => val <= 0.38 ? (int)Grades.A : val <= 0.50 ? (int)Grades.B : val <= 0.63 ? (int)Grades.C : val <= 0.75 ? (int)Grades.D : (int)Grades.F);
                }
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    for (int r = 0; r < gridSize; r++)
                    {
                        for (int c = 0; c < gridSize; c++)
                        {
                            ...
                            {
                                0 => baseGridR[r, c],
                                1 => baseGridR[gridSize - 1 - c, r],
                                2 => baseGridR[gridSize - 1 - r, gridSize - 1 - c],
                                _ => baseGridR[c, gridSize - 1 - r]
                            };
                ...
                int gradeSC = 4;
                if (roiObject?.UserConfiguredGrading?.SymbolContrast?.IsEnabled == true)
                {
                    gradeSC = GetGrades(scPercent, roiObject.UserConfiguredGrading.SymbolContrast.GradingData,
                        val => val >= 70 ? (int)Grades.A : val >= 55 ? (int)Grades.B : val >= 40 ? (int)Grades.C : val >= 20 ? (int)Grades.D : (int)Grades.F);
                }
                ...
                int gradeMOD = 4;
                if (roiObject?.UserConfiguredGrading?.Modulation?.IsEnabled == true)
                {
                    gradeMOD = GetGrades(worstMod, roiObject.UserConfiguredGrading.Modulation.GradingData,
                        val => val >= 0.40 ? 4 : val >= 0.30 ? 3 : val >= 0.20 ? 2 : val >= 0.10 ? 1 : 0);
                }

                // =========================================================================
                // 6. FINAL GRADES & NEW FIXES
                // =========================================================================
                int gradeFPD = 4;
                if (roiObject?.UserConfiguredGrading?.FixedPatternDamage?.IsEnabled == true)
                {
                    gradeFPD = GetGrades(bestFpdErrors, roiObject.UserConfiguredGrading.FixedPatternDamage.GradingData,
                        val => val == 0 ? (int)Grades.A : val <= 1 ? (int)Grades.B : val <= 2 ? (int)Grades.C : val <= 3 ? (int)Grades.D : (int)Grades.F);
                }

                int gradeDecode = isDecoded ? 4 : 0;
                //if (roiObject?.UserConfiguredGrading?.Decode?.IsEnabled == true)
                //{
                //    //// Map decode status to numeric: 1.0 (Success) or 0.0 (Fail)
                //    //double decodeValue = isDecoded ? 1.0 : 0.0;

                //    //// Grade the decodeValue using user configuration ranges
                //    //gradeDecode = GetGrades(decodeValue, roiObject.UserConfiguredGrading.Decode.GradingData,
                //    //    val => val >= 1.0 ? 4 : 0);
                //}

                int gradeUEC = 4;
                if (roiObject?.UserConfiguredGrading?.UnusedErrorCorrection?.IsEnabled == true)
                {
                    // Grade based on visualErrors directly using user configuration ranges
                    gradeUEC = GetGrades(visualErrors, roiObject.UserConfiguredGrading.UnusedErrorCorrection.GradingData,
                        val => val <= 2 ? 4 : 2);
                }
                ...
                if (!cornersFound)
                {
                    ...
                    {
                        Point[] hull = Cv2.ConvexHull(contours.OrderByDescending(c => Cv2.ContourArea(c)).First());
                        ptTL = GetExtremeCorner(hull, 1, 1);
                        ptBR = GetExtremeCorner(hull, -1, -1);
                        ptTR = GetExtremeCorner(hull, -1, 1);
                        ptBL = GetExtremeCorner(hull, 1, -1);
                    }
                }
                ...
                int gradeQZ = 4;
                if (roiObject?.UserConfiguredGrading?.QuietZone?.IsEnabled == true)
                {
                    gradeQZ = GetGrades(qzModules, roiObject.UserConfiguredGrading.QuietZone.GradingData,
                        val => val >= 0.4 ? 4 : 0);
                }
                ...
                return new GS1_QC_CheckResult
                (
                    Decode: roiObject?.UserConfiguredGrading?.Decode?.IsEnabled == true ? letters[gradeDecode] : null,
                    SC: roiObject?.UserConfiguredGrading?.SymbolContrast?.IsEnabled == true ? $"{letters[gradeSC]} ({(scPercent):F2}%)" : null,
                    AN: roiObject?.UserConfiguredGrading?.AxialNonuniformity?.IsEnabled == true ? $"{letters[gradeAN]} ({axialNonUniformity:F2})" : null,
                    GN: roiObject?.UserConfiguredGrading?.GridNonuniformity?.IsEnabled == true ? $"{letters[gradeGN]} ({gnCells:F2} cell)" : null,
                    MOD: roiObject?.UserConfiguredGrading?.Modulation?.IsEnabled == true ? letters[gradeMOD] : null,
                    FPD: roiObject?.UserConfiguredGrading?.FixedPatternDamage?.IsEnabled == true ? letters[gradeFPD] : null,
                    UEC: roiObject?.UserConfiguredGrading?.UnusedErrorCorrection?.IsEnabled == true ? $"{letters[gradeUEC]}" : null,
                    PG: $"R_Max: {(rMax / 255.0 * 100):F2}%, R_Min: {(rMin / 255.0 * 100):F2}%",
                    OverAll: $"{letters[finalGrade]}",
                    AS9132_Distortion: $"{angleDistortion:F2}°",
                    AS9132_QuietZone: roiObject?.UserConfiguredGrading?.QuietZone?.IsEnabled == true ? $"{qzPercent:F2}% ({letters[gradeQZ]})" : null,
                    AS9132_Elongation: $"{(axialNonUniformity * 100):F2}%",
                    DPM_Rmin: $"R_Min: {(rMin / 255.0 * 100):F2}%"
                );
            }
        ...
        /// Evaluates grade based on configured ranges or defaults.
        /// </summary>
        private int GetGrades(double value, List<GradingRange> gradingRanges, Func<double, int> defaultGradeCompute)
        {
            // If user did not provide custom ranges, safely fall back to the defaults
            if (gradingRanges == null || gradingRanges.Count == 0)
            {
                return defaultGradeCompute(value);
            }
            ...
            var greaterEqualRanges = gradingRanges
                .Where(r => r.ComparisonOperators == ComparisonOperators.GreaterThanEqualTo)
                .OrderByDescending(r => r.GradeValue)
                .ToList();

            var lessEqualRanges = gradingRanges
                .Where(r => r.ComparisonOperators == ComparisonOperators.LessThanEqualTo)
                .OrderBy(r => r.GradeValue)
                .ToList();

            foreach (var range in greaterEqualRanges)
            {
                if (value >= range.GradeValue) return (int)range.Grades;
            }
